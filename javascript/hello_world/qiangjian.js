(function () {
    ...
    for (i = 0, len = checkbox.length; i < len; i++) {
        ...
        if (weibo.test(name) || kelaite.test(name) || xinbei.test(name) || kuaiji.test(name) || liren.test(name) || mage.test(name) || xinbei.test(name) || yingyu.test(name) || hengqi.test(name) || xinshijie.test(name) || yuanrun.test(name) || jingying.test(name) || feicui.test(name)) {
